...
//    private int roomCount = 0;
    ...
    void Start()
    {
        ...
        int spawnRoomID = Random.Range(0, allRoomCoords.Count);
        Room spawnRoom = rooms[allRoomCoords[spawnRoomID]];
        AssignRooms(spawnRoomID);
        PopulateCurrentRoom(spawnRoom);
        SpawnPlayer();
    }
    ...
    public void DeleteOldEnvironment()
    {
        ...
        foreach (GameObject g in GameObject.FindGameObjectsWithTag("GoblinBullet"))
        {
            Destroy(g);
        }
    }
...
//                Debug.Log(coords);
    ...
    private void AssignRooms(int id)
    {
        ...
        for (int i = 0; i < allRoomCoords.Count; i++)
        {
            ...
            if(i==id)//if i is equal to the id passed in from the start method determined to be the starting room
            {
                rooms[allRoomCoords[i]].roomType = 1; //set first room to starting room
            }
            else
            {
